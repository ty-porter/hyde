...
class Interpreter(Visitor):
    ...
    def visit_super(self, expr):
        ...
        if method is None:
            raise InterpreterRuntimeError(expr.method, f"Undefined property '{expr.method.lexeme}'.")
    ...
    def visit_classdef(self, stmt):
        ...
        if stmt.superclass is not None:
            superclass = self.visit(stmt.superclass)
            if not isinstance(superclass, HydeClass):
                raise InterpreterRuntimeError(stmt.superclass.name, 'Superclass must be a class.')
        ...
        klass = HydeClass(stmt.name.lexeme, superclass, methods)
